...
function wave_aff_view_overview( $d ) {
	...
	echo '<section class="wa-panel" data-wa-limit="10" data-wa-all="' . ( $all ? '1' : '0' ) . '"><div class="wa-section-head"><div><h2>Top affiliates</h2><p>Top 10 for the selected ranking. Search includes every affiliate.</p></div><label>Rank by <select data-wa-sort><option value="earned">Commission earned</option><option value="visits">Traffic</option><option value="linked">Customers attached</option><option value="converted">Converted visits</option></select></label></div>';
...
function wave_aff_view_reports( $d ) {
	...
	wave_aff_form_start( 'report', $d['month'] ); echo '<button class="button button-primary" ' . disabled( $d['limited'], true, false ) . '>Save report &amp; prepare downloads</button> <a class="button" href="' . esc_url( wave_aff_native( 'payouts' ) ) . '">Check past payouts ↗</a></form>';
